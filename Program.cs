...
void IncluirNovaTarefa()
{
    ...
    {
        UI.ExibeErro("Não é possível incluir a tarefa sem descrição.");
        return;
    }
    using (var _db = new tarefasContext())
    {
...
void AlterarDescricaoDaTarefa()
{
   ...
   if (String.IsNullOrEmpty(descricao))
    {
        UI.ExibeErro("Não é possível continuar sem uma descrição.");
        return;
    }
     using (var _db = new tarefasContext())
    {
...
void ConcluirTarefa()
{
     ...
     using (var _db = new tarefasContext())
    {
        ...
        if(tarefa.Concluida)
        {
            UI.ExibeErro("Tarefa já foi concluída");
            return;
        }
